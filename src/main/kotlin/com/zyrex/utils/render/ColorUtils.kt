...
object ColorUtils {
    ...
    fun isAllowedCharacter(character: Char) =
        character.toInt() != 167 && character.toInt() >= 32 && character.toInt() != 127
    ...
    val minecraftRed = Color(255, 85, 85)
    ...
    fun randomMagicText(text: String): String = buildString(text.length) {
        for (c in text) {
            if (isAllowedCharacter(c)) {
                val index = (0 until allowedCharArray.size).random()
    ...
    fun interpolateHealthColor(
        ...
        healthFromScoreboard: Boolean, absorption: Boolean
    ): Color {
...
private fun getHealth(entity: EntityLivingBase, healthFromScoreboard: Boolean, absorption: Boolean): Float {
